...
export type PersistenceType = 'json' | 'postgres';
 ...
 * When PERSISTENCE_TYPE is 'json' (default), DATABASE_URL is completely optional.
...
export function validateConfig(env: NodeJS.ProcessEnv = process.env): ConfigValidationResult {
  ...
  if (rawPersistence === 'postgres' || rawPersistence === 'postgresql') {
    ...
    errors.push(
      `Invalid PERSISTENCE_TYPE '${env.PERSISTENCE_TYPE}'. Supported values are 'json' (default) or 'postgres'.`
    );
  }

    ...
    <header className="sticky top-0 z-40 w-full bg-[#171717] text-[#F7F5EF] border-b border-[#171717]/40 font-mono text-xs shadow-xs shrink-0">
      ...
      <div className="max-w-7xl mx-auto px-2.5 sm:px-4 lg:px-6 py-1.5 sm:py-2 flex items-center justify-between gap-1.5 sm:gap-3">
        ...
        <div className="flex items-center space-x-1.5 sm:space-x-2.5 min-w-0 shrink">
          ...
          <div className="cursor-pointer min-w-0 max-w-[110px] sm:max-w-none truncate" onClick={() => onNavigate?.('overview')}>
            <span className="font-serif font-bold text-xs sm:text-base tracking-wide text-white uppercase truncate block">
              {t('brand.name')}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1 sm:gap-2 shrink-0">
            ...
            <>
              ...
              <div className="relative md:hidden shrink-0">
                <button
                  ...
                  className="flex items-center gap-1 px-1.5 py-1 bg-[#285943]/70 hover:bg-[#285943] border border-emerald-500/50 text-emerald-200 rounded-xs text-[11px] transition-colors cursor-pointer shrink-0"
                  title="Account options"
                  aria-label="Account options"
                  aria-expanded={accountMenuOpen}
                >
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
                  <span className="text-[10px] font-sans font-medium text-white max-w-[48px] sm:max-w-[70px] truncate">
                ...
                {accountMenuOpen && (
                  <div 
                    className="absolute right-[-4.5rem] sm:right-0 top-full mt-1.5 w-[min(264px,calc(100vw-1.25rem))] max-w-[calc(100vw-1.25rem)] bg-[#1F1E1A] border border-white/20 shadow-2xl z-50 p-3 text-[#F7F5EF] rounded-xs font-sans"
                    role="dialog"
                    aria-label="Account options"
                  >
                    <div className="flex items-center justify-between pb-2 border-b border-white/10 mb-2 gap-1.5">
                      <div className="flex items-center gap-1.5 min-w-0">
                        <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
                        <span className="text-[10px] font-bold text-emerald-300 uppercase tracking-wider font-mono truncate">
                          Authenticated Citizen
                        </span>
                      </div>
                      <button
                        onClick={() => setAccountMenuOpen(false)}
                        className="text-slate-400 hover:text-white p-1 -mr-1 cursor-pointer shrink-0 rounded-xs hover:bg-white/10"
                    ...
                    <div className="space-y-1.5 mb-3 text-[11px] min-w-0">
                      {user.displayName && (
                        <div className="font-semibold text-white break-words overflow-hidden line-clamp-2 leading-snug">
                          {user.displayName}
                        </div>
                      )}
                      {user.email && (
                        <div 
                          className="text-[10px] text-slate-300 font-mono break-all leading-tight text-slate-300/90"
                          title={user.email}
                        >
                          {user.email}
                        </div>
                      )}
                      <div className="text-[10px] text-emerald-400/90 font-mono pt-1 leading-tight break-words">
                    ...
                    <button
                      ...
                      className="w-full min-h-[34px] py-1.5 px-3 bg-red-950/50 hover:bg-red-900/70 active:bg-red-900 border border-red-500/40 text-red-200 hover:text-white text-xs font-mono font-semibold rounded-xs transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
                    >
          ...
          <div className="relative shrink-0">
            ...
            {langDropdownOpen && (
              <div 
                className="absolute right-[-1.5rem] sm:right-0 top-full mt-1.5 w-[min(230px,calc(100vw-1.25rem))] max-w-[calc(100vw-1.25rem)] bg-white border border-slate-300 shadow-xl z-50 py-1 text-slate-900 rounded-xs"
                role="dialog"
                aria-label="Select language"
              >
                <div className="px-3 py-1.5 border-b border-slate-100 text-[10px] font-bold text-slate-500 uppercase tracking-wider bg-slate-50 truncate">
                ...
                <div className="max-h-[60vh] overflow-y-auto py-0.5">
                  {supportedLanguages.map((lang) => (
                    ...
                    >
                      <span className="truncate pr-2">{lang.nativeName} {lang.name !== lang.nativeName && <span className="text-slate-500 text-[11px]">({lang.name})</span>}</span>
                      {language === lang.code && <Check className="w-3 h-3 text-[#D65A3A] shrink-0" />}
                    </button>
                  ))}
                </div>
              </div>
